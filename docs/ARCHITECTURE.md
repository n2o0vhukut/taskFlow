...
- 課題: チェックインで入力した内容が TaskFlow のDB（/tasks）に反映されていない／反映されたか分かりづらい
  - 事象例:
    - 「新規: …」「[x] …」「延期: …」と入力しても、ダッシュボードにタスク追加・完了・期日変更が見えない
    - 反映結果（add/update/done/defer）の件数や失敗の理由がユーザーに返らない
  - 改善案:
    - 抽出ルールの強化（行頭キーワード・記号の網羅、箇条書き検出の閾値調整、日本語表現の追加）
    - 適用結果の可視化: プランDMの冒頭に `Applied: add X / update Y / done Z / defer W` を表示、失敗は `errors:` として明示
    - 監査の拡張: `ai_audit.jsonl` に `apply` 成功/失敗を別ログで追記、`/admin/trigger {"type":"drain"}` で再送可能である旨をDMにも案内
    - 事前プレビュー: モーダル送信後に「適用予定の差分（プレビュー）」を一度DMし、ユーザーが「適用する」ボタンを押すと `/ai/apply` 実行（Dry-Run → Apply）
    - ダッシュボード改善: 直近適用イベントのタイムライン（誰が・いつ・何を）と、期限<=48h/OVERLOADの集計を追加
    - 失敗時の復旧: スプール再送を手動起動できるUI（ダッシュボードから）と、`/admin/trigger` の結果表示

- 課題: 同様のタスクがダッシュボード上で重複して表示される（AIの重複統合が十分に効いていない）
  - 事象例:
    - タイトル表記ゆれ（例: 「機能A 実装」「機能A 実装 Part2」「機能Ａ実装」）が別タスクとして増殖
    - チェックイン由来の「新規: …」が既存にマージされず別IDで追加される
  - 改善案:
    - 類似度判定の強化：Jaccardしきい値のチューニング、正規化（全角半角/記号/カナ）、日本語形態素解析ベースのトークン化
    - LLM/埋め込みの併用：OpenAIのEmbeddingsでタイトル類似度を補助（クラスタリング→代表タスクへ正規化）
    - 連携フローの改善：`organize` で `dedupe` に canonical_title/target_task_id を付与し、`apply` で rename/merge を実施
    - ユーザー確認フロー：重複疑い（0.7–0.8帯）はDMで確認ボタン（「統合する/別タスク」）→選択結果を適用
    - ダッシュボード機能：重複クラスタの一覧（代表/候補/類似度）と「一括統合」操作
...
Must（直ちに着手）
- チェックイン入力のDB反映と可視化強化
  - 抽出: 日本語表現（新規/完了/延期/不可時間）ルールの拡張、箇条書き検出の閾値調整
  - 適用: DM冒頭に `Applied: add X / update Y / done Z / defer W` と `errors:` の要約を表示
  - 監査: `ai_audit.jsonl` に apply 成功/失敗を区別して追記（payload/対象ID/理由）
  - UX: 事前プレビュー（Dry‑Run）→「適用する」ボタンで `/ai/apply` 実行（誤適用を回避）
  - 受け入れ基準: 入力→差分が /tasks に反映、DM/監査に件数・理由が記録、失敗時はスプール/再送で回復

- 重複タスク（dedupe）の統合精度向上
  - 正規化: 全角半角・記号除去・カナ/漢字揺れの緩和、日本語トークン化
  - 類似度: Jaccardしきい値の再調整（例: ≥0.8 で自動統合、0.7–0.8 は確認フロー）
  - 処理: `organize.dedupe` に canonical_title/target_task_id を付与、`apply` で rename/merge を実施
  - 確認: DMに「統合する/別タスク」ボタン（曖昧帯のみ）
  - 受け入れ基準: 同義の表記揺れが代表タスクに統合、誤検知は確認フローで回避

- プランDMの可読性・説明責任の向上
  - 表示: ブロックに期限/優先度バッジ、合計時間、上位理由（S_total要旨）を併記
  - サマリ: DM冒頭に「総タスク/本日計画/期限<=48h件数/警告件数」を表示
  - 受け入れ基準: プランだけ見て「何を/いつ/なぜ/どれくらい」が理解できる

- Slack DM/コマンド運用の安定化
  - 設定: DM許可/App Home、Interactivity/Slash/Manifest のURL統一、必要なら `app_mention`/`message.im` を購読
  - 受け入れ基準: `/plan` とDM/モーダルが常時動作、URL/署名/権限の不整合が解消

Should（次フェーズ）
- 全体タスク俯瞰・期限可視化
  - ダッシュボード: 期限/優先度フィルタ、期限<=48hの集計とハイライト
  - Slack: `/tasks` で要約（総数/TODO/期限<=48h/今週）
  - 受け入れ基準: 「全体量/期限リスク」が1クリックで把握可能

- 優先度（スコア）の可視化
  - 可視化: S_base/S_adj/S_total のグラフ、/plan 上位3件のスコア＋一言理由
  - 受け入れ基準: 優先順位の根拠が数値と理由で追える

- 随時整理の導線（自然文→整理→適用→プラン）
  - 機能: `/organize`、メッセージアクション「整理する」、DMキーワード発火
  - 受け入れ基準: 任意タイミングの指示で整理が駆動し、差分がDBに反映される

Could（後続改善）
- 学習時間の記録と可視化（`/log Nh テーマ`、ダッシュボード週次推移）
- 重複クラスタUI（代表/候補/類似度の一覧と一括統合）
- 再送/エラー復旧UI（ダッシュボードから drain 実行・結果表示）

Next 3（直近の実装提案）
1. プランDM改善（Applied件数・警告サマリ・期限/優先度バッジ）
2. 抽出/適用の可視化（Dry‑Run→Apply、apply成否をDMと監査へ）
3. 重複統合強化（正規化＋しきい値再調整、曖昧帯はDM確認）

**チェックリスト**
- Must項目の受け入れ基準が定義されている
- 次に実装する3点が明確である
- 設計（API/UI/ログ）に落とし込み可能な粒度になっている
